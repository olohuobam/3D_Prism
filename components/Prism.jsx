...
export default function Prism({ scrollProgress = 0 }) {
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      ...
      if (scrollProgress >= 0.63) {
        // Mobile: centered, slightly smaller; desktop: normal
        const maxS  = isMobile ? 1.0 : 1.2
        const s     = lerp(0.3, maxS, assembleT)
        groupRef.current.scale.set(s, s, s)

        // Center on mobile, slight left offset on desktop
        groupRef.current.position.x = isMobile ? 0 : 0
    ...
    segRefs.forEach((ref, i) => {
      ...
      // Slightly reduced separation on mobile
      const mob = isMobile ? 0.7 : 1.0
      ref.current.position.x = lerp(0, t.posX * mob, sep)
      ref.current.position.y = lerp(0, t.posY * mob, sep)
    ...
    if (scanRef.current) {
      ...
      scanRef.current.material.opacity = (0.45 + Math.sin(time * 0.6) * 0.10) * assembleT
    }

    // ── Expanding square projection plane ─────────────────────────────────
  ...
  return (
    <group ref={groupRef} scale={[1.2, 1.2, 1.2]} visible={false}>
      ...
      {/* Vivid cyan square projection plane */}
